...
<head>
    ...
    <style>
        ...
        .complete-btn { background: #805528; color: white !important; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-size: 14px; font-weight: bold; display: inline-block; transition: background 0.3s; }
        .complete-btn:hover { background: #a06b33; }
        .done-text { color: green; font-weight: bold; font-size: 14px; padding: 10px 20px; display: inline-block; border: 1px solid transparent; }
    </style>
</head>
<body>
        ...
        <div class="item-card">
            ...
            <div class="actions">
                ...
                <?php if (!empty($item['request_id'])): ?>
                    <button type="button" onclick="openReportModal(<?= $item['request_id'] ?>, <?= $item['donation_id'] ?>)">
                        <img src="images/report-btn.png" style="width: 25px; height: 25px;">
                    </button>
                <?php endif; ?>
            </div>
        </div>
...
<div id="reportModal" style="display:none; position:fixed; z-index:1000; left:0; top:0; width:100%; height:100%; background-color:rgba(0,0,0,0.5);">
    <div style="background-color:white; margin:15% auto; padding:20px; width:300px; border-radius:8px; position:relative; color:black;">
        ...
        <p onclick="submitReport('Spam or misleading information.')">Spam or misleading information.</p>
        <p onclick="submitReport('User who behave inappropriately.')">User who behave inappropriately.</p>
    </div>
</div>
